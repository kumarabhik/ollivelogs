"""OTel bootstrap for log-consumer worker. Soft no-op if OTLP endpoint missing."""
...
SERVICE_NAME: Final = "log-consumer"


class WorkerTracer(Protocol):
...
class JsonLogFormatter(logging.Formatter):
    def format(self, record: logging.LogRecord) -> str:
        ...
        for key in ("stream_id", "status", "reason"):
...
def get_worker_tracer() -> WorkerTracer:
    """Return a tracer for the worker loop (no-op if OTel not installed)."""
    try:
        from opentelemetry import trace

        return cast(WorkerTracer, trace.get_tracer("log-consumer.worker"))
    except ImportError:
        class _NoOp:
            def start_as_current_span(
                self,
        ...
        return cast(WorkerTracer, _NoOp())
